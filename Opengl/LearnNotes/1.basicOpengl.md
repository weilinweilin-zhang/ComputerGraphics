...
图元
#### 着色器的坐标系统
在计算机图形学中，顶点着色器和片段着色器中使用的坐标系统有所不同，并且这两个着色器处理的坐标不完全是我们日常理解的标准坐标。
##### 顶点着色器中的坐标
在顶点着色器中，坐标通常以对象空间或模型空间的形式输入。这些坐标随后通过一系列的变换矩阵（如模型矩阵、视图矩阵和投影矩阵）转换为裁剪空间坐标：
- **对象空间（Object Space）**：原始模型顶点所在的坐标系。
- **世界空间（World Space）**：将对象空间的顶点变换到世界环境中的坐标系。
- **视图空间（View Space）**：也称相机空间，顶点变换到相机视角的坐标系。
- **裁剪空间（Clip Space）**：应用投影矩阵以后，顶点定位的坐标系。
顶点着色器的主要任务之一是将对象空间的顶点坐标变换到裁剪空间坐标系。这些裁剪空间坐标随后会被传递给光栅化阶段。
### 片段着色器中的坐标
在片段着色器中，输入的坐标通常是通过光栅化阶段从先前顶点着色器的输出插值而来。在此阶段，常用的坐标有：
- **屏幕空间坐标（Screen Space Coordinates）**：这些是标准化设备坐标（NDC）映射到屏幕的2D坐标，通常用于像素操作和屏幕空间效果。
- **纹理坐标（Texture Coordinates）**：用于采样纹理的坐标，这些坐标通常在顶点着色器中设置并传递到片段着色器。
### 标准化设备坐标（NDC）
在顶点着色器之后，经过光栅化之前，坐标被变换到标准化设备坐标（NDC）：
- **NDC空间**：坐标在[-1, 1]范围内，适合进行屏幕映射。
总结起来，顶点着色器处理的是从对象空间转换到裁剪空间的坐标，而片段着色器处理的通常是屏幕空间或纹理空间坐标。这些坐标不是我们传统意义上的“标准”坐标系统，而是特定于计算机图形学渲染管线的坐标系统。
...
// 正常默认的是采用纹理单元0
先设置着色器里面的 对应哪个纹理单元，然后再绘制的过程中给他绑定具体加载的纹理id即可完成采样
